...
class DoctorRequestScreen extends ConsumerStatefulWidget {
  const DoctorRequestScreen({super.key});

  @override
  ConsumerState<DoctorRequestScreen> createState() => _DoctorRequestScreenState();
}

class _DoctorRequestScreenState extends ConsumerState<DoctorRequestScreen> {
  int _currentStep = 1;
  ...
  void _nextStep(AppLocalizations l10n) {
    ...
    if (_currentStep == 1) {
      if (interventionType == null) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text(l10n.chooseInterventionType)),
        );
        return;
      }
    }
    
    if (interventionType == InterventionType.appointment && _currentStep == 3) {
    ...
    if (interventionType == InterventionType.appointment && _currentStep == 4) {
      if (state.currentRequest?.details.interventionDetails.appointmentDateTime == null) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text(l10n.chooseTimeSlot ?? 'Veuillez choisir un créneau horaire')),
        );
        return;
      }
    }
  ...
  Widget build(BuildContext context) {
    ...
    // SECURITE : Attendre l'initialisation du bon type pour éviter le crash de cast
    if (sosState.currentType != SosType.doctor || sosState.currentRequest?.details is! DoctorDetails) {
          ...
          children: [
            ...
            Padding(
              ...
              child: MsdButton(
                ...
                isLoading: ref.watch(sosProvider).isLoading,
              ),
            ),
          ],
        ),
  ...
  Widget _buildStepContent(AppLocalizations l10n) {
    ...
    if (interventionType == InterventionType.appointment) {
      switch (_currentStep) {
        case 1: return _buildRequestDetailsStep(l10n);
  ...
  Widget _buildRequestDetailsStep(AppLocalizations l10n) {
    final sosState = ref.watch(sosProvider);
    final details = sosState.currentRequest?.details as DoctorDetails?;

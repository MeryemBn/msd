...
class _AmbulanceRequestScreenState extends ConsumerState<AmbulanceRequestScreen> {
    ...
    Future.microtask(() {
      ref.read(sosProvider.notifier).initNewRequest(
            AmbulanceDetails(
              ...
              ambulanceType: AmbulanceType.medicalisee_smur,
            ),
            500.0,
            SosType.ambulance,
          );
  ...
  Widget build(BuildContext context) {
    ...
    if (sosState.currentType != SosType.ambulance || details is! AmbulanceDetails) {
  ...
  Widget _buildStepContent(AppLocalizations l10n, AmbulanceDetails details) {
  ...
  Widget _buildRequestDetailsStep(AppLocalizations l10n, AmbulanceDetails details) {
    final interventionType = details.interventionDetails.interventionType;
    ...
    return SingleChildScrollView(
      child: Column(
        ...
        children: [
          ...
          Text(l10n.specifyAmbulanceType, style: TextStyle(color: isDark ? Colors.white60 : Colors.grey, fontSize: 14)),
          const SizedBox(height: 24),
          
          SosDropdownField<AmbulanceType>(
            label: l10n.ambulanceType,
            hint: l10n.chooseAmbulanceType,
            value: details.ambulanceType,
            items: AmbulanceType.values,
            itemLabelBuilder: (a) => a.getLabel(l10n),
            prefixIcon: Icons.emergency_outlined,
            onChanged: (val) {
              if (val != null) ref.read(sosProvider.notifier).updateAmbulanceType(val);
            },
          ),
  ...
  Widget _buildAvailableSlotsStep() {
    ...
    final details = sosState.currentRequest?.details as AmbulanceDetails?;
